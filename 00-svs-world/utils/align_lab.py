...
# 1) singing voice databse's mono-phone labels and
# 2) generated labels by sinsy

ph2num = prep_ph2num()

sinsy_files = sorted(glob(join(config.out_dir, "sinsy_mono_round/*.lab")))
mono_label_round_files = sorted(glob(join(config.out_dir, "mono_label_round/*.lab")))

dst_dir = join(config.out_dir, "mono_dtw")
os.makedirs(dst_dir, exist_ok=True)

excludes = []
for (path1, path2) in zip(sinsy_files, mono_label_round_files):
    lab_sinsy = hts.load(path1)
    lab_mono_label_round = hts.load(path2)
    name = basename(path1)
    if name in excludes:
        print("Skip!", name)
        continue
    ...
    d, path = fastdtw(ph2numeric(lab_sinsy.contexts,ph2num),
        ph2numeric(lab_mono_label_round.contexts, ph2num), radius=len(lab_mono_label_round))

    # Edit sinsy labels with hand-annontated aligments
    for x, y in path:
        lab_sinsy.start_times[x] = lab_mono_label_round.start_times[y]
        lab_sinsy.end_times[x] = lab_mono_label_round.end_times[y]
